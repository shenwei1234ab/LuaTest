...
int add111(lua_State* L)
{
	int a = lua_tointeger(L, 1);
	int b = lua_tointeger(L, 2);


	int     datas = lua_tointeger(L, lua_upvalueindex(1));

	void*   dd = lua_touserdata(L, lua_upvalueindex(2));
	int*    pInt = (int*)dd;
...
int main(int argc, char** argv)
{
	...
	int     iData = 123456;
	int     funcSize = sizeof(iData);

	lua_pushinteger(L, 300);
	void*   data = lua_newuserdata(L, funcSize);
	stackDump(L);
	memcpy(data, &iData, funcSize);

	lua_pushcclosure(L, add111, 2);
	stackDump(L);
	lua_setglobal(L, "addExt");
	stackDump(L);
	luaL_dostring(L, "addExt(1,2)");
